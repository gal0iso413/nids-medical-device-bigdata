...
describe("published Class 1 generation path", () => {
  it("loads canonical insufficient data through current manifest then generation files", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn((url:string)=>Promise.resolve({ok:true,json:()=>Promise.resolve(url.endsWith("current.json") ? current : url.endsWith("internal-service.json") ? fixture.class1.service : fixture.class1.graph)})) as unknown as typeof fetch);
