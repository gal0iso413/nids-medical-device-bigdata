...
import type {
  ...
  SelectionItem,
  SelectionType,
} from "./contracts/class2Mock";
import type {
  Class2AnalysisPayload,
  Class2SelectionCatalogRow,
} from "./contracts/class2Analysis";
import {
  resolveCurrentClass2PageState,
  type Class2PageState,
...
function localPeriod(analysis: Class2AnalysisPayload): PeriodState {
...
function statusMessage(state: Class2PageState): string {
...
function statusTone(state: Class2PageState): string {
  if (state.kind === "error") {
    return "danger";
  }
  if (state.kind === "local_analysis") {
...
export default function App({ initialState }: AppProps) {
  const [state, setState] = useState<Class2PageState>(
  ...
  useEffect(() => {
    ...
    let active = true;
    void resolveCurrentClass2PageState().then((nextState) => {
      if (active) {
        setState(nextState);
      }
    });
  ...
  const candidateSelections: LocalSelectionItem[] = localAnalysis
    ? localAnalysis.selection_catalog.map((selection: Class2SelectionCatalogRow) => ({
