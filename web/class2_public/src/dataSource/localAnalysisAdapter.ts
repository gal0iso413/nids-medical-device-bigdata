import {
  class2AnalysisSchemaVersion,
  type Class2AnalysisPayload,
  type Class2SelectionCatalogRow,
  type Class2SelectionCoverageSummaryRow,
  type Class2SelectionMonthCompositionRow,
  type Class2SelectionMonthMetricRow,
} from "../contracts/class2Analysis";
...
function catalogRow(value: unknown, index: number): Class2SelectionCatalogRow {
  ...
  return { selection_id: string(row.selection_id, "selection_id")!, selection_type: type, label: string(row.label, "label")!, normalized_label: string(row.normalized_label, "normalized_label")!, source_labels: strings(row.source_labels, "source_labels"), parent_item_group_selection_id: string(row.parent_item_group_selection_id, "parent_item_group_selection_id", true), parent_item_group_label: string(row.parent_item_group_label, "parent_item_group_label", true), parent_conflict_status: parentStatus as Class2SelectionCatalogRow["parent_conflict_status"], quality_flags: string(row.quality_flags, "quality_flags")! };
}

function metricsRow(value: unknown, index: number): Class2SelectionMonthMetricRow {
...
function compositionRow(value: unknown, index: number): Class2SelectionMonthCompositionRow {
...
export function validateClass2AnalysisPayload(value: unknown): Class2AnalysisPayload {
  const payload = record(value, "payload");
  if (payload.analysis_schema_version !== class2AnalysisSchemaVersion) throw new Error(`Unsupported analysis_schema_version: ${String(payload.analysis_schema_version)}.`);
  return { analysis_schema_version: class2AnalysisSchemaVersion, selection_catalog: array(payload.selection_catalog, "selection_catalog").map(catalogRow), selection_month_metrics: array(payload.selection_month_metrics, "selection_month_metrics").map(metricsRow), selection_month_composition: array(payload.selection_month_composition, "selection_month_composition").map(compositionRow), selection_coverage_summary: array(payload.selection_coverage_summary, "selection_coverage_summary").map(coverageRow) };
}

export async function loadLocalClass2Analysis(url: string, request = fetch): Promise<Class2AnalysisPayload> {
  const response = await request(url);
  if (!response.ok) throw new Error(`Local analysis request failed (${response.status}).`);
  return validateClass2AnalysisPayload(await response.json());
}
