...
```powershell
python -m data_pipeline.offline.class3_analysis_export --config C:/local/class3-export-config.json
```

The command atomically publishes only these ignored files:

```text
web/class3_public/public/generated/class3-analysis.json
web/class3_public/public/generated/class3-analysis-manifest.json
```
...
Or set `VITE_CLASS3_ANALYSIS_URL` to a separately hosted local JSON file. The browser reads JSON only; JSON generation remains this offline command's responsibility. `local` mode is not a public-service approval or release state.
