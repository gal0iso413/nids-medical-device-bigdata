# Class 3 serving marts

`data_pipeline.analysis.class3_serving_mart` is an offline batch builder. It
...
```powershell
python -m data_pipeline.analysis.class3_serving_mart `
  --fact-root "D:\NIDS Local Run\facts" `
  --output-root "D:\NIDS Local Run\class3-serving-marts" `
  --period-start 202401 `
  --period-end 202406
```

`python -m data_pipeline.cli.class3_serving_mart` is an equivalent standalone
...
separate `tools/offline/requirements-class3-serving-mart-win-py313.lock` wheel
...
query endpoints, public-serving policy, and the Class 3 React API-mode change are
