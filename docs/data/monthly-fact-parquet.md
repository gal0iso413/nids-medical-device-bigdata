...
publication atomicity는 월 파티션 하나를 단위로 한다. 여러 월을 한 번에 쓰는 호출은 전체 트랜잭션이 아니므로 앞선 월이 정상 게시된 뒤 후속 월이 실패할 수 있다. 이 경우 이미 게시된 정상 월은 유지하며 rollback하거나 삭제하지 않는다. 같은 입력으로 다시 실행하면 정상 월은 no-op으로 확인하고 아직 게시되지 않은 나머지 월을 이어서 처리한다.
