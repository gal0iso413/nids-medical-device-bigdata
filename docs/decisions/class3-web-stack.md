...
> 상태: **Locked decision**
>
> 적용 대상: Class 3 공개 웹 `web/class3_public/`
>
> 적용 시작: PR-C3-UI-01A
>
...
- **Locked decision** — Class 3 최종 웹은 React, Vite, TypeScript `strict` 모드로 구현한다.
- **Locked decision** — 패키지 관리자는 npm을 사용하고 lockfile은 `package-lock.json`으로 고정한다.
- **Locked decision** — 신규 웹의 대상 경로는 `web/class3_public/`이다.
...
- **Locked decision** — 현재 Class 3 범위에는 Next.js를 채택하지 않는다.
...
- [목표 웹 아키텍처](../architecture/target-web-architecture.md)
