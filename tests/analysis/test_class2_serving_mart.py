...
from data_pipeline.analysis.class2_serving_mart import (
...
class Class2ServingMartTests(unittest.TestCase):
    def setUp(self) -> None:
        self.root = Path(tempfile.mkdtemp(prefix="class2-serving-mart-"))
    ...
    def build(self, **kwargs: object):
        return build_class2_serving_marts(
    ...
    def test_different_content_conflicts_and_staging_is_cleaned(self) -> None:
        ...
        with self.assertRaises(Class2ServingMartConflictError):
            build_class2_serving_marts(fact_root=different, output_root=self.output_root, period_start="202401", period_end="202402")
    ...
    def test_invalid_period_missing_input_has_bounded_diagnostics_root_overlap_and_failed_staging_are_safe(self) -> None:
        with self.assertRaisesRegex(Class2ServingMartError, "period_start"):
            build_class2_serving_marts(fact_root=self.fact_root, output_root=self.root / "bad", period_start="202402", period_end="202401")
        with self.assertRaisesRegex(Class2ServingMartError, "partitions must verify") as raised:
            build_class2_serving_marts(fact_root=self.fact_root, output_root=self.root / "missing", period_start="202401", period_end="202403")
        self.assertNotIn(str(self.root), str(raised.exception))
        self.assertNotIn("supplier-secret", str(raised.exception))
        with self.assertRaisesRegex(Class2ServingMartError, "must not overlap fact_root"):
            build_class2_serving_marts(fact_root=self.fact_root, output_root=self.fact_root, period_start="202401", period_end="202402")
        with self.assertRaisesRegex(Class2ServingMartError, "must not overlap checkpoint_root"):
            build_class2_serving_marts(fact_root=self.fact_root, output_root=self.root / "checkpoint", checkpoint_root=self.root / "checkpoint", period_start="202401", period_end="202402")
        import data_pipeline.analysis.class2_serving_mart as builder
        failed_root = self.root / "failed"
        with patch.object(builder, "_copy_query", side_effect=OSError("synthetic write failure")):
            with self.assertRaises(Class2ServingMartError):
                build_class2_serving_marts(fact_root=self.fact_root, output_root=failed_root, period_start="202401", period_end="202402")
    ...
    def test_reader_returns_tx_hhi_and_overlap_without_endpoint_hashes(self) -> None:
        from services.class2_local_api.reader import MartReader
        from services.class2_local_api.schemas import ComparisonSelection
    ...
    def test_omitted_period_covers_all_available_fact_months(self) -> None:
        result = build_class2_serving_marts(fact_root=self.fact_root, output_root=self.output_root)
        self.assertEqual(result.status, "written")
        manifest = json.loads((result.manifest_path).read_text(encoding="utf-8"))
        self.assertEqual(manifest["period_start"], "202401")
        self.assertEqual(manifest["period_end"], "202402")
