...
from services.class2_local_api.app import create_app
from services.class2_local_api.reader import MartVerificationError
...
class Class2LocalApiTests(unittest.TestCase):
    def setUp(self) -> None:
        self.root = Path(tempfile.mkdtemp(prefix="class2-api-"))
        ...
        build_class2_serving_marts(fact_root=self.facts, output_root=self.marts, period_start="202401", period_end="202402")
    ...
    def manifest_path(self) -> Path:
        return self.marts / "class2_serving_mart" / f"schema_version={SERVING_MART_SCHEMA_VERSION}" / "_manifest.json"
    ...
    def test_comparison_returns_tx_composition_hhi_and_period_overlap(self) -> None:
        ...
        self.assertEqual(overlap["supplier_union_count"], 2)
        self.assertEqual(overlap["pairs"][0]["supplier_intersection_count"], 1)
