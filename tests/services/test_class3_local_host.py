...
class Class3LocalIntegratedHostTests(unittest.TestCase):
    def setUp(self) -> None:
        self.root = Path(tempfile.mkdtemp(prefix="class3-local-host-"))
        ...
        build_class3_serving_marts(fact_root=self.facts, output_root=self.marts, period_start="202401", period_end="202401")
        (self.static / "assets").mkdir(parents=True)
        (self.static / "index.html").write_text("<!doctype html><title>Class 3 local</title><div id=app></div>", encoding="utf-8")
    ...
    def test_static_deep_links_and_existing_api_are_served_by_one_origin(self) -> None:
        ...
        self.assertNotIn("Class 3 local", api_missing.text)
