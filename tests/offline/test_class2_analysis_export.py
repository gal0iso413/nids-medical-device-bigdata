...
from data_pipeline.contracts.class2_analysis import serialize_class2_analysis
...
from data_pipeline.offline.class2_analysis_export import (
    Class2OfflineExportConflictError,
    Class2OfflineExportError,
    Class2SelectionRequest,
    export_class2_analysis,
)
...
class Class2OfflineExportTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def export(self, **kwargs):
        return export_class2_analysis(
            parquet_root=self.store, period_start="202401", period_end="202402",
            selections=(Class2SelectionRequest("item_group", "Group A"),), web_public_root=self.public,
            **kwargs,
        )

    def payload(self) -> dict[str, object]:
        return json.loads((self.public / "generated" / "class2-analysis.json").read_text(encoding="utf-8"))
    ...
    def test_multiple_selection_period_serializer_and_adapter_shape(self) -> None:
        result = export_class2_analysis(
            ...
            selections=(Class2SelectionRequest("item_group", "Group A"), Class2SelectionRequest("item_group", "Group B")),
        )
    ...
    def test_payload_retains_existing_serializer_table_contract_except_local_metadata(self) -> None:
        self.export()
        payload = self.payload()
        expected = serialize_class2_analysis(build_class2_analysis(
    ...
    def test_lineage_conflict_blocks_overwrite(self) -> None:
        ...
        with self.assertRaises(Class2OfflineExportConflictError):
            export_class2_analysis(
                parquet_root=other_store, period_start="202401", period_end="202402", web_public_root=self.public,
                selections=(Class2SelectionRequest("item_group", "Group A"),),
            )

    def test_item_name_selection_keeps_its_parent_scope(self) -> None:
        result = export_class2_analysis(
            parquet_root=self.store, period_start="202401", period_end="202402", web_public_root=self.public,
            selections=(Class2SelectionRequest("item_name", "Item B", "Group B"),),
        )
    ...
    def test_item_name_selection_requires_parent_scope(self) -> None:
        with self.assertRaisesRegex(Class2OfflineExportError, "requires parent_item_group_label"):
            export_class2_analysis(
                parquet_root=self.store, period_start="202401", period_end="202402", web_public_root=self.public,
                selections=(Class2SelectionRequest("item_name", "Item B"),),
            )
    ...
    def test_payload_write_failure_leaves_no_publish_and_retry_writes(self) -> None:
        import data_pipeline.offline.class2_analysis_export as exporter

        with patch.object(exporter, "_atomic_write", side_effect=Class2OfflineExportError("synthetic payload failure")):
            with self.assertRaisesRegex(Class2OfflineExportError, "synthetic payload failure"):
                self.export()
        self.assertFalse((self.public / "generated" / "class2-analysis.json").exists())
    ...
    def test_manifest_failure_recovers_matching_payload_then_is_unchanged(self) -> None:
        ...
        def fail_manifest(path: Path, data: bytes) -> None:
            if path.name == "class2-analysis-manifest.json":
                raise Class2OfflineExportError("synthetic manifest failure")
            real_write(path, data)

        with patch.object(exporter, "_atomic_write", side_effect=fail_manifest):
            with self.assertRaisesRegex(Class2OfflineExportError, "synthetic manifest failure"):
                self.export()
        payload_path = self.public / "generated" / "class2-analysis.json"
        manifest_path = self.public / "generated" / "class2-analysis-manifest.json"
    ...
    def test_empty_selection_suppressed_and_not_available_payloads(self) -> None:
        empty = export_class2_analysis(parquet_root=self.store, period_start="202401", period_end="202402", selections=(), web_public_root=self.public)
        ...
        no_rows = export_class2_analysis(parquet_root=self.store, period_start="202401", period_end="202402", selections=(Class2SelectionRequest("item_group", "Absent Group"),), web_public_root=no_rows_public)
        self.assertEqual(no_rows.export_state, "not_available")
        self.assertEqual(json.loads((no_rows_public / "generated" / "class2-analysis.json").read_text())["local_export"]["reason"], "no_rows_for_requested_selections")
        second_public = self.root / "second-public"
        suppressed = export_class2_analysis(parquet_root=self.store, period_start="202401", period_end="202402", selections=(Class2SelectionRequest("item_group", "Group A"),), web_public_root=second_public, availability_state="suppressed")
        self.assertEqual(suppressed.export_state, "suppressed")
        self.assertEqual(json.loads((second_public / "generated" / "class2-analysis.json").read_text())["local_export"]["public_policy_state"], "not_applied")
