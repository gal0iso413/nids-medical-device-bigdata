"""Focused contract tests for the Class 2 local analysis data product."""
...
class Class2AnalysisContractTests(unittest.TestCase):
    def build(self, rows: list[dict[str, object]], start: str = "202401", end: str = "202401"):
        return build_class2_analysis(
    ...
    def test_input_order_does_not_change_tables(self) -> None:
        ...
        self.assertEqual(serialize_class2_analysis(first), serialize_class2_analysis(second))
    ...
    def test_serializer_is_deterministic_and_excludes_forbidden_fields(self) -> None:
        payload = serialize_class2_analysis(self.build([{}]))
        self.assertEqual(payload["analysis_schema_version"], CLASS2_ANALYSIS_SCHEMA_VERSION)
        self.assertEqual(payload, serialize_class2_analysis(self.build([{}])))
