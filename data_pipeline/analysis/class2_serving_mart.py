"""Build immutable Class 2 serving marts from verified monthly fact Parquet.
...
SERVING_MART_DATASET_NAME: Final = "class2_serving_mart"
...
class Class2ServingMartError(RuntimeError):
    """Raised when a serving mart cannot be safely built."""


class Class2ServingMartConflictError(Class2ServingMartError):
    """Raised rather than replacing an existing serving mart with new content."""


@dataclass(frozen=True)
class Class2ServingMartResult:
...
def _months(period_start: str, period_end: str) -> tuple[str, ...]:
    if not isinstance(period_start, str) or not isinstance(period_end, str):
        raise Class2ServingMartError("period bounds must be YYYYMM strings")
    ...
    try:
        ...
        raise Class2ServingMartError("period bounds are invalid") from exc
    months = tuple(period.strftime("%Y%m") for period in periods)
    if not months or months[0] != period_start or months[-1] != period_end:
        raise Class2ServingMartError("period_start must not be after period_end")
    return months
...
def _validate_roots(fact_root: Path, output_root: Path, checkpoint_root: Path | None) -> None:
    ...
    if _is_within(output_root, fact_root) or _is_within(fact_root, output_root):
        raise Class2ServingMartError("output_root must not overlap fact_root")
    if checkpoint_root is not None and (
        _is_within(output_root, checkpoint_root) or _is_within(checkpoint_root, output_root)
    ):
        raise Class2ServingMartError("output_root must not overlap checkpoint_root")
...
def _write_candidate(
    ...
    try:
        ...
        for month in months:
            ...
            if frame.empty:
                raise Class2ServingMartError(f"verified partition {month} has no aggregate observations")
            loaded = frame.copy()
    ...
    except Class2ServingMartError:
        raise
    except Exception as exc:
        raise Class2ServingMartError("could not build Class 2 serving marts") from exc
    finally:
        connection.close()
...
def _read_existing_manifest(final_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise Class2ServingMartConflictError("existing serving mart manifest is unreadable") from exc
    if not isinstance(value, dict) or raw != _canonical_json_bytes(value):
        raise Class2ServingMartConflictError("existing serving mart manifest is not canonical")
    return value
...
def _new_staging_dir(final_dir: Path) -> Path:
    ...
    for _ in range(10):
        ...
        try:
            ...
            continue
        return candidate
    raise Class2ServingMartError("could not allocate serving mart staging directory")


def build_class2_serving_marts(
    *,
    fact_root: Path,
    output_root: Path,
    ...
    if (period_start is None) != (period_end is None):
        raise Class2ServingMartError("period_start and period_end must both be set or both omitted")
    if period_start is None:
        months = discover_monthly_fact_months(fact_root)
        if not months:
            raise Class2ServingMartError("no verified monthly fact partitions are available")
    ...
    try:
        ...
        raise Class2ServingMartError("all requested monthly fact partitions must verify") from exc
    ...
    try:
        ...
        try:
            ...
            raise Class2ServingMartError("could not atomically publish serving marts") from exc
        return Class2ServingMartResult(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build Class 2 serving marts from verified monthly facts.")
    ...
    result = build_class2_serving_marts(
