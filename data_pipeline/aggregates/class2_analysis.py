"""Pure Class 2 analysis tables derived from the shared monthly fact."""
...
def _selection_id(selection_type: str, normalized_label: str, parent_id: str = "") -> str:
    ...
    return f"c2s:v1:{selection_type}:{sha256(payload).hexdigest()}"
...
def _month_range(period_start: str, period_end: str) -> tuple[str, ...]:
    if not isinstance(period_start, str) or not isinstance(period_end, str):
        raise Class2AnalysisContractError("period bounds must be YYYYMM strings")
    ...
    try:
        ...
        raise Class2AnalysisContractError("period bounds are invalid") from exc
    if not len(months):
        raise Class2AnalysisContractError("period bounds are invalid")
    ...
    for value in values:
        if not _is_missing(value):
            if not isinstance(value, Decimal):
                raise Class2AnalysisContractError("monthly fact decimals must remain Decimal")
            total += value
            found = True
    ...
    if not composition.empty:
        ...
        for _, group in grouped:
            if int(group["endpoint_count"].sum()) != int(group["denominator_endpoint_count"].iloc[0]):
                raise Class2AnalysisContractError("composition denominator invariant failed")
    return composition
...
def build_class2_analysis(
    ...
    *,
    ...
    """Build deterministic Class 2 local analysis tables from a monthly fact."""
    validated = validate_monthly_fact(fact)
    months = _month_range(period_start, period_end)
    if not isinstance(data_version, str):
        raise Class2AnalysisContractError("data_version must be a non-blank string")
    normalized_data_version = _clean_label(data_version)
    if normalized_data_version is None:
        raise Class2AnalysisContractError("data_version is required")
    ...
    return validate_class2_analysis_tables(
        Class2AnalysisTables(catalog, metrics, composition, coverage)
    )
