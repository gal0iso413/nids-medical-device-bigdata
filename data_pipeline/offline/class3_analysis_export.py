"""Publish a local-only Class 3 analysis JSON from verified monthly partitions.

The browser consumes the PR #15 serializer payload at the existing
``/generated/class3-analysis.json`` adapter path.  This module is intentionally
...
PAYLOAD_FILENAME: Final = "class3-analysis.json"
MANIFEST_FILENAME: Final = "class3-analysis-manifest.json"
DEFAULT_WEB_PUBLIC_ROOT: Final = Path("web/class3_public/public")
...
class Class3OfflineExportError(RuntimeError):
    """Raised when a local analysis export cannot be safely produced."""


class Class3OfflineExportConflictError(Class3OfflineExportError):
    """Raised rather than replacing an existing different local artifact."""


@dataclass(frozen=True)
class Class3SelectionRequest:
...
def _months(period_start: str, period_end: str) -> tuple[str, ...]:
    if not _MONTH_PATTERN.fullmatch(period_start) or not _MONTH_PATTERN.fullmatch(period_end):
        raise Class3OfflineExportError("period bounds must be YYYYMM")
    try:
        periods = pd.period_range(period_start, period_end, freq="M")
    except ValueError as exc:
        raise Class3OfflineExportError("period bounds are invalid") from exc
    if not len(periods):
        raise Class3OfflineExportError("period bounds are invalid")
    return tuple(period.strftime("%Y%m") for period in periods)


def _validate_selections(selections: Iterable[Class3SelectionRequest]) -> tuple[Class3SelectionRequest, ...]:
    result: list[Class3SelectionRequest] = []
    seen: set[tuple[str, str, str | None]] = set()
    for selection in selections:
        if selection.selection_type not in {"item_group", "item_name"}:
            raise Class3OfflineExportError("selection_type must be item_group or item_name")
...
def _filter_fact(fact: pd.DataFrame, selections: tuple[Class3SelectionRequest, ...]) -> pd.DataFrame:
    if not selections:
...
def _requested_selection_ids(
    catalog: pd.DataFrame, selections: tuple[Class3SelectionRequest, ...],
) -> set[str]:
...
def _filter_analysis(tables: Class3AnalysisTables, selection_ids: set[str]) -> Class3AnalysisTables:
...
def _atomic_write(path: Path, data: bytes) -> None:
    ...
    try:
        ...
        raise Class3OfflineExportError(f"could not atomically publish {path.name}") from exc
...
def _existing_result(
    ...
    try:
        ...
        raise Class3OfflineExportConflictError("existing local export is unreadable; refusing overwrite") from exc
    if raw_manifest != _canonical_json_bytes(manifest):
        raise Class3OfflineExportConflictError("existing local export manifest is not canonical; refusing overwrite")
    if manifest.get("payload_sha256") != payload_sha:
        raise Class3OfflineExportConflictError("existing local export checksum disagrees with manifest")
    if manifest.get("export_fingerprint") != export_fingerprint:
        raise Class3OfflineExportConflictError("existing local export has different lineage or selection; refusing overwrite")
    return Class3OfflineExportResult("unchanged", payload_path, manifest_path, payload_sha, str(manifest["export_state"]))


def export_class3_analysis(
    *,
    parquet_root: Path,
    period_start: str,
    period_end: str,
    selections: Iterable[Class3SelectionRequest],
    web_public_root: Path = DEFAULT_WEB_PUBLIC_ROOT,
    availability_state: Literal["available", "suppressed", "not_available"] = "available",
) -> Class3OfflineExportResult:
    ...
    the generated artifact under ``web/class3_public/public/generated``.
    """
    if availability_state not in _STATES:
        raise Class3OfflineExportError("availability_state is invalid")
    ...
    if availability_state == "suppressed":
        ...
        if not available_months:
            ...
            if selected_fact.empty:
                ...
                payload = serialize_class3_analysis(_filter_analysis(tables, selection_ids))
...
def _read_config(path: Path) -> dict[str, Any]:
    try:
        ...
        raise Class3OfflineExportError("config must be readable UTF-8 JSON") from exc
    if not isinstance(value, dict):
        raise Class3OfflineExportError("config root must be an object")
    return value


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Export local-only Class 3 analysis JSON.")
    parser.add_argument("--config", type=Path, required=True)
    args = parser.parse_args(argv)
    config = _read_config(args.config)
    selections = tuple(Class3SelectionRequest(**item) for item in config.get("selections", []))
