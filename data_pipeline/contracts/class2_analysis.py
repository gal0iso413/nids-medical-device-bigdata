"""Typed, in-memory contract for the Class 2 local analysis data product."""
...
CLASS2_ANALYSIS_SCHEMA_VERSION: Final = "1.0.0"
...
class Class2AnalysisContractError(ValueError):
    """Raised when a Class 2 analysis table violates its typed contract."""


@dataclass(frozen=True)
class Class2AnalysisTables:
    """All in-memory tables produced by :func:`build_class2_analysis`."""
...
def _validate_table(name: str, frame: pd.DataFrame, columns: tuple[str, ...]) -> None:
    if tuple(frame.columns) != columns:
        raise Class2AnalysisContractError(f"{name} columns do not match the contract")
    for column in _STRING_COLUMNS[name]:
        if str(frame[column].dtype) != "string":
            raise Class2AnalysisContractError(f"{name}.{column} must use string dtype")
    for column in _COUNT_COLUMNS.get(name, ()):
        if str(frame[column].dtype) != "Int64":
            raise Class2AnalysisContractError(f"{name}.{column} must use Int64 dtype")
    for column in _DECIMAL_COLUMNS.get(name, ()):
        if str(frame[column].dtype) != "object":
            raise Class2AnalysisContractError(f"{name}.{column} must use object Decimal dtype")
    if name == "selection_month_composition" and str(frame["is_unknown"].dtype) != "boolean":
        raise Class2AnalysisContractError("selection_month_composition.is_unknown must use boolean dtype")


def validate_class2_analysis_tables(tables: Class2AnalysisTables) -> Class2AnalysisTables:
...
def serialize_class2_analysis(tables: Class2AnalysisTables) -> dict[str, Any]:
    """Return a deterministic JSON-compatible representation without writing files."""
    validate_class2_analysis_tables(tables)
    return {
