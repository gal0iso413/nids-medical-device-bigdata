"""Run the Class 1 lookup API only on a local loopback interface."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the local-only Class 1 lookup API.")
    ...
    app = (
        create_app(args.index_root)
        if args.static_root is None
        else create_integrated_app(args.index_root, args.static_root)
    )
