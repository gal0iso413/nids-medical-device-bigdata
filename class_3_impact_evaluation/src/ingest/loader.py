"""
Top7 data loader — Class 3 Impact Evaluation & Public Report.
Same top7 files; .0 suffix stripping is CRITICAL for this agent (keys are string type).
"""
...
def _resolve_slacker():
    try:
        load_dotenv(REPO_ROOT / "class_3_impact_evaluation" / ".env")
        import sys
        sys.path.insert(0, str(REPO_ROOT))
        from shared_utils.slacker import AgentSlacker
        return AgentSlacker("class_3_impact_evaluation")
    except Exception:
        return None
...
def load_master(*, verbose: bool = True, validate_keys: bool = True) -> pd.DataFrame:
    ...
    p = profile_df(df, "top7_master [class3]")
    if verbose:
        log_profile(p)
    if validate_keys:
...
def load_supply(
    *, verbose: bool = True, validate_keys: bool = True, cap_prices: bool = True
    ...
    p = profile_df(df, f"top7_supply [class3] [{sheet}]")
    if verbose:
        log_profile(p)
    if validate_keys:
